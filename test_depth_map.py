...
class DepthMap: 
    ...
    def computeDepthMapSGBM(self): 
        ...
        for cnt in contours:
            ...
            self.imgDetected = cv.putText(self.imgDetected, 'Depth : '+str(round(depth, 2)), (10, 40), cv.FONT_HERSHEY_SIMPLEX , 1, (0, 255, 0), 2, cv.LINE_AA) 
            disparity_colored = cv.putText(disparity_colored, 'Depth : '+str(round(depth, 2)), (10, 40), cv.FONT_HERSHEY_SIMPLEX , 1, (0, 255, 0), 2, cv.LINE_AA)  
            blended_image = cv.putText(blended_image, 'Depth : '+str(round(depth, 2)), (10, 40), cv.FONT_HERSHEY_SIMPLEX , 1, (0, 255, 0), 2, cv.LINE_AA)   
            cv.rectangle(disparity_colored, (x, y), (x + w, y + h-20), (0, 0, 255), 3)
            cv.imwrite('detect.png', self.imgDetected[y:y+h, x:x+w])
